            ...
            notes.append(
                {
                    "key": ex["key"],
                    "name": sub_name,
                    "note": f"Замена «{ex['name']}» — записано, прогрессию по нему не двигаем",
                    "working_weight": None,
                }
            )
            continue
...
def supp_payload(u) -> dict:
    ...
    return {
        ...
        # кнопка "отметить приём" имеет смысл, только если человек вообще
        # отметил хоть одну добавку как принимаемую — раньше было жёстко
        # завязано на креатин конкретно, что не давало отметиться тому, кто
        # принимает, например, только омега-3 и витамин D.
        "any_taken": bool(taken),
    }
